...
Route::post('trails', function (Request $request) {
    DB::insert('INSERT INTO trails (name, location) VALUES (?, ?)', [$request->name, $request->location]);
    $trails = DB::select('SELECT * FROM trails ORDER BY id ASC');
    return $trails;
});
...
Route::put('trails/{id}', function (Request $request, $id) {
    DB::update('UPDATE trails SET name=?, location=? WHERE id = ?', [$request->name, $request->location, $id]);
    $trails = DB::select('SELECT * FROM trails ORDER BY id ASC');
    return $trails;
});
